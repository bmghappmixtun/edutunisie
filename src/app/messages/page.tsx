      ...
      <main className="flex-1 pt-24 pb-10 bg-slate-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 mb-6">
            <Link href={user.role === 'TEACHER' ? '/enseignant' : '/mon-compte'} className="p-2 hover:bg-slate-100 rounded-lg">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <span className={`truncate ${conv.unread > 0 ? 'font-bold text-slate-900' : 'font-semibold text-slate-700'}`}>
                          {otherName}
                        </span>
                        {lastMsg && (
                      ...
                      {lastMsg ? (
                        <p className={`text-sm truncate mt-0.5 ${conv.unread > 0 ? 'text-slate-700' : 'text-slate-500'}`}>
                          {lastMsg.senderId === user.id && <span className="text-slate-400">Vous : </span>}
                          {lastMsg.content}
                        </p>
                      ) : (
