...
import {
  ...
  ChevronDown, Calendar, FileWarning, Filter, Sparkles,
} from 'lucide-react';
...
const SECTION_DISPLAY: Record<string, { icon: string; nameFr: string; nameAr: string; color: string }> = {
  math: { icon: '📐', nameFr: 'Mathématiques', nameAr: 'الرياضيات', color: 'blue' },
  'sc-exp': { icon: '🧪', nameFr: 'Sciences Expérimentales', nameAr: 'العلوم التجريبية', color: 'emerald' },
  'sc-tech': { icon: '⚙️', nameFr: 'Sciences Techniques', nameAr: 'العلوم التقنية', color: 'slate' },
  'sc-info': { icon: '💻', nameFr: 'Sciences Informatiques', nameAr: 'علوم الإعلامية', color: 'indigo' },
  'eco-gestion': { icon: '💼', nameFr: 'Économie et Gestion', nameAr: 'الاقتصاد والتصرف', color: 'amber' },
...
export function BacArchivesClient({
          ...
          for (const [session, files] of Object.entries(sessions)) {
            ...
            if (searchQuery) {
              ...
              filtered = filtered.filter((f: any) => {
                const searchStr = `${yg.year} ${section} ${session} ${f.subject} ${f.type} ${f.subjectName || ''}`.toLowerCase();
  ...
  }, [yearGroups, yearFilter, sectionFilter, subjectFilter, sessionFilter, typeFilter, searchQuery]);

  const totalFiltered = filteredGroups.reduce((s, yg) =>
    s + Object.values(yg.sections).reduce((ss, sessions) =>
  ...
  const hasActiveFilters = searchQuery || yearFilter || sectionFilter || subjectFilter || sessionFilter || typeFilter;
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row gap-3">
            <div className="flex-1 relative">
              ...
              <input
                type="search"
                placeholder={isAr ? 'بحث في الأرشيف (مادة، شعبة، سنة...)' : 'Rechercher (matière, section, année...)'}
                value={searchQuery}
            ...
            <select value={sectionFilter} onChange={(e) => setSectionFilter(e.target.value)}
              ...
              {availableSections.map((s) => (
                <option key={s} value={s}>
                  {SECTION_DISPLAY[s]?.icon} {isAr ? SECTION_DISPLAY[s]?.nameAr : SECTION_DISPLAY[s]?.nameFr}
                </option>
              ))}
            </select>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mt-3">
            <div className="text-sm text-slate-600">
              {hasActiveFilters ? (
                <span>
                  <strong className="text-violet-700">{totalFiltered.toLocaleString()}</strong> {isAr ? 'ملف يطابق البحث' : 'fichiers trouvés'}
                  <span className="text-slate-400"> / {totalFiles.toLocaleString()}</span>
                </span>
              ) : (
                <span>
                  <strong className="text-violet-700">{totalFiles.toLocaleString()}</strong> {isAr ? 'ملف في' : 'fichiers sur'} <strong>{yearGroups.length}</strong> {isAr ? 'سنة' : 'ans'}
                </span>
              )}
            </div>
          ...
          {filteredGroups.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-slate-600 mb-4">
                {isAr ? 'حاول تعديل المرشحات للعثور على ما تبحث عنه.' : 'Essayez de modifier les filtres pour trouver ce que vous cherchez.'}
              </p>
              <button onClick={clearFilters}
                        ...
                        <div>
                          ...
                          <div className="text-xs text-slate-500 flex items-center gap-3 flex-wrap">
                            <span className="inline-flex items-center gap-1">
                              <Calendar className="w-3 h-3" /> {yg.total} {isAr ? 'ملف' : 'fichiers'}
                            </span>
                            <span className="text-slate-300">•</span>
                            <span>{yg.sujets} {isAr ? 'موضوع' : 'sujets'}</span>
                            <span className="text-slate-300">•</span>
                            <span className="text-amber-600 font-semibold">{yg.corriges} {isAr ? 'إصلاح' : 'corrigés'}</span>
                            <span className="text-slate-300">•</span>
                            <span>{sectionCount} {isAr ? 'شعب' : 'sections'}</span>
                          </div>
                        </div>
                      </div>
                      <ChevronDown className={`w-5 h-5 text-slate-400 transition-transform ${isOpen ? 'rotate-180' : ''} flex-shrink-0`} />
                    </summary>

                    {isOpen && (
                    <div className="px-5 pb-5 border-t border-slate-100 pt-4 space-y-4">
                      {Object.entries(yg.sections).map(([sectionSlug, sessions]) => {
                        const sec = SECTION_DISPLAY[sectionSlug] || { icon: '📄', nameFr: sectionSlug, nameAr: sectionSlug, color: 'slate' };
                        return (
                          <div key={sectionSlug} className="bg-gradient-to-br from-slate-50 to-white rounded-xl p-4 border border-slate-100">
                            {/* Section header */}
                            ...
                            <div className="space-y-3">
                              {Object.entries(sessions).map(([session, files]) => (
                                <div key={session} className="bg-white rounded-lg p-3 border border-slate-100">
                                  <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className="text-xs text-slate-500">{files.length} {isAr ? 'ملف' : 'fichiers'}</span>
                                  </div>
                                      ...
                                      return (
                                        ...
                                        >
                                          <span className="flex items-center gap-1.5 min-w-0 flex-1">
                                            {isCorrige ? <CheckCircle className="w-3.5 h-3.5 flex-shrink-0" /> : <FileText className="w-3.5 h-3.5 flex-shrink-0" />}
                                            <span className="truncate">{subjectLabel(f.subject || '', f.subjectName, f.subjectNameAr)}</span>
                                            {isCorrige && <Sparkles className="w-3 h-3 flex-shrink-0" />}
                                          </span>
