 ...
 * If the title contains Arabic text at the end (after " - " separator or directly after space),
 ...
 *   splitArabicSubject("Devoir de Contrôle - الفرض العادي عدد 2")
 *     -> ["Devoir de Contrôle", "الفرض العادي عدد 2"]
 */
export function splitArabicSubject(title: string | null | undefined): {
  ...
  // First try with " - " separator before Arabic block
  let match = title.match(/^(.*?)\s+-\s+([\u0600-\u06FF][\u0600-\u06FF\s،.؟!]*)$/);
  if (match) {
    return { fr: match[1].trim(), ar: match[2].trim() };
  }
  // Then try with just a space before Arabic block at the end
