...
async function main() {
  ...
  // ============ LEVELS ============
  const primaire = await prisma.level.create({
    data: { slug: 'primaire', nameFr: 'Primaire', nameAr: 'الابتدائي', order: 1 }
  });
  const college = await prisma.level.create({
    data: { slug: 'college', nameFr: 'Collège', nameAr: 'الإعدادي', order: 2 }
  });
  const lycee = await prisma.level.create({
    data: { slug: 'lycee', nameFr: 'Lycée', nameAr: 'الثانوي', order: 3 }
  });
  ...
  const teachers = await Promise.all(
    teachersData.map(t => prisma.user.create({
      data: {
        ...
        teachingLevels: JSON.stringify(t.levels),
  ...
  const students = await Promise.all(
    studentsData.map(s => prisma.user.create({
      data: {
        email: s.email,
        passwordHash,
        role: 'STUDENT',
        ...
        await prisma.comment.create({
          data: {
            ...
            content: [
              ...
              'Très utile pour mes révisions du Bac.',
              'Le corrigé est très détaillé, parfait pour comprendre.',
              'Bonne qualité, je recommande !',
              'M\'a beaucoup aidé pour préparer mon contrôle.',
            ][i % 5],
          }
        });
      }
  ...
  console.log(`   - ${await prisma.user.count()} users (${teachers.length} teachers, ${students.length} students, 1 admin)`);
